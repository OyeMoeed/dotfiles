...
import { StatusSuccessQuaternaryVariantProps } from './status-succes-quaternary-variant-component.interface';
import statusSuccessQuaternaryVariantStyles from './status-succes-quaternary-variant-component.style';

const StatusSuccessQuaternaryVariant: React.FC<StatusSuccessQuaternaryVariantProps> = ({ variantProps }) => {
  const { headingText, descriptionText, onPressGoToCard, onPressHome } = variantProps;

  const { colors } = useTheme();
  const styles = statusSuccessQuaternaryVariantStyles(colors);
      ...
      <IPayLinearGradientView gradientColors={backgroundGradientColors}>
        ...
        <IPayView style={styles.footerView}>
          <IPayButton
            ...
            rightIcon={<IPayIcon icon={icons.rightArrow} color={colors.primary.primary500} size={18} />}
          />
          <IPayButton
            btnType={buttonVariants.OUTLINED}
...
export default StatusSuccessQuaternaryVariant;

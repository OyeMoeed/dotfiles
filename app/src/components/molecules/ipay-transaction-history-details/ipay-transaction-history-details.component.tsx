...
const IPayTransactionHistoryDetails: React.FC<IPayTransactionHistoryDetailsProps> = ({
  testID,
  style,
  transactionData,
}) => {
  ...
  const [transactionDataArry, setTransactionDataArray] = useState<{ key: string; value: any }[]>([]);
  ...
  const renderTransactionDetails = ({ item, index }: ItemProps) => (
    <IPayView style={[styles.transactionCard, (index === 9 || index === 17) && styles.transactionCardConditionalStyle]}>

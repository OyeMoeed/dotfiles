...
import prepareForgetPasscode from './prepare-forget-passcode.service';

// Mock the apiCall function
jest.mock('@network/services/api-call.service');

describe('prepareForgetPasscode', () => {
  const mockApiResponse = { ok: true, data: { response: { otpRef: '123456', walletNumber: 'mockWallet' } } };
  ...
  it('returns the API response if it is successful', async () => {
    ...
    // Call the prepareForgetPasscode function
    const result = await prepareForgetPasscode(mockPayload, mockDispatch);
    ...
    expect(mockDispatch).toHaveBeenCalledWith(setAppData({ otpRef: '123456', walletNumber: 'mockWallet' }));
  });

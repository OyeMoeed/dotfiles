...
import getWUBeneficiaryInfoMetaData from './wu-beneficiary-details-metadata.service';
...
jest.mock('../international-transfer.urls', () => ({
  western_union_beneficiaries: jest.fn(),
}));
jest.mock('./wu-beneficiary-details-metadata.mock');

describe('getWUBeneficiaryInfoMetaData', () => {
  ...
  it('should return mock data when MOCK_API_RESPONSE is true', async () => {
    ...
    const result = await getWUBeneficiaryInfoMetaData('');
    expect(result).toBe(wuBeneficiaryDetailsMetaDataMock);
  });
  ...
  it('should return { apiResponseNotOk: true } when api response is not ok', async () => {
    ...
    (INTERNATIONAL_TRANSFERS_URLS.western_union_beneficiaries as jest.Mock).mockReturnValue('url');
